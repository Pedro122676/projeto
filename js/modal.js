...
function abrirModalEdicao(tipo, item) {
  ...
  let titulo = `Editar ${tipo.charAt(0).toUpperCase() + tipo.slice(1)}`;
  let conteudo = '';

  if (tipo === 'consulta') {
    conteudo = `
      <div class="mb-3"><label class="form-label">Paciente</label><input type="text" id="modalPaciente" class="form-control" value="${item.paciente || ''}"></div>
      <div class="mb-3"><label class="form-label">Dentista</label><input type="text" id="modalDentista" class="form-control" value="${item.dentista || ''}"></div>
      <div class="row">
        <div class="col-6 mb-3"><label class="form-label">Data</label><input type="date" id="modalData" class="form-control" value="${item.data || ''}"></div>
        <div class="col-6 mb-3"><label class="form-label">Hora</label><input type="time" id="modalHora" class="form-control" value="${item.hora || ''}"></div>
      </div>
  ...
  document.querySelectorAll('#editModal').forEach(el => el.remove());
...
async function salvarEdicao() {
  ...
  try {
    ...
    if (res.ok) {
      showToast(`${currentEditType} atualizado com sucesso!`, 'sucesso');
      
      if (typeof window[`init${currentEditType.charAt(0).toUpperCase() + currentEditType.slice(1)}s`] === 'function') {
        window[`init${currentEditType.charAt(0).toUpperCase() + currentEditType.slice(1)}s`]();
      } else if (currentEditType === 'estoque' && typeof initEstoque === 'function') initEstoque();
    } else {
